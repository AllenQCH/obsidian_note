...
## 2026-07-18：上下文压缩风暴修复

### 现象与根因

一次 Feishu 长任务运行到 48/90 轮后，新消息进入时触发了连续上下文压缩。现场证据显示：

- 活跃上下文约 `96,400 tokens`，旧配置的压缩阈值只有 `64,000`。
- 压缩连续创建多个 child session，但消息数量出现 `112 -> 112`、`120 -> 120`、`126 -> 126`、`130 -> 130`、`132 -> 132`，没有产生有效缩减。
- `model.context_length` 配成 `128000`，运行时模型目录却把 `gpt-5.6-sol` 识别为 `400000`，造成阈值口径不一致。
- 新消息中断旧任务后，递归 follow-up 使用了压缩前的 parent session id，再次把旧上下文带回执行链。
- 最近消息和工具输出保护过大：`protect_last_n=20`、单次工具输出上限 `50000 bytes`，使可保留尾部本身就可能超过 64k 阈值。

### 当前修复

本机 `~/.hermes/config.yaml` 已调整为：
...
对应效果：自动压缩阈值从 `64k` 恢复为 `200k`，减少工具输出和最近消息对活动上下文的长期占用。

运行时代码同时增加两条保护：

1. 压缩前后使用同一套“完整请求”口径计算 token；消息数未下降且节省不足 10% 时，立即触发 anti-thrashing 熔断，不再连续压缩。
2. 中断后的 follow-up 使用结果中最新的 child session id，不再回绑旧 parent session。

Gateway 已通过 launchd 重启，PID 完成切换，Feishu WebSocket 重新连接。重启后恢复的旧 session 约 81k input tokens，低于新的 200k 阈值，直接执行模型调用，没有再次预压缩。

### 使用约定

- 只创建空白新会话：使用 `/new <标题>`。
- 新建会话并立即执行新任务：使用 `/task <任务内容>`。
- 长任务逻辑不能中断、但不想继续携带完整聊天记录：使用 `/handoff <标题>`。Hermes 会先整理当前目标、进度、关键事实、决策、待办和下一步，摘要成功后切换到新 session 并继续执行；摘要失败时保留原 session。
- 需要留在同一 session 内主动压缩：仍可使用 `/compact <focus>`，但它不是隔离原始聊天记录的首选方案。
- 长时间执行且不应阻塞聊天入口：使用 `/background <任务内容>`。
- 群聊只作为入口；单个 session 对应一个任务；稳定事实写入 Memory、Obsidian 或 Skill，不依赖无限延长聊天记录。

### Feishu 会话生命周期

- 每日 `09:00 Asia/Shanghai` 主动轮换现有 Feishu 群聊和私聊 session；轮换时不主动刷消息，下一次对话会提示已进入每日新 session。
- 如果 09:00 时某个 session 正在执行任务，不硬中断；任务结束后 watcher 会补做轮换。
- 自动压缩阈值当前为 `200,000 tokens`，达到其 `70%`（`140,000 tokens`）后，每个已完成回复都会追加一次上下文提示，并同时解释 `/handoff`、`/new`、`/task`、`/background` 的使用区别。

### 个人事务 Role

本地配置、个人自动化、文件整理、浏览器操作、知识维护及其他非业务杂事使用：

`solve_personal_problem`

该 Role 不等同于迭代开发，不默认进入业务仓库、OpenSpec、分支、测试、commit/push 或流水线；它由 Router 继续复用现有 stage 与 `tool_*` 能力完成具体操作。

### 维护提醒

Launchd 的工作目录是 `~/.hermes`，实际导入的是：

- `~/.hermes/hermes-agent/venv/lib/python3.11/site-packages/agent/conversation_compression.py`
- `~/.hermes/hermes-agent/venv/lib/python3.11/site-packages/gateway/run.py`

本次同时同步了 `~/.hermes/hermes-agent/` 下的开发副本和 `site-packages` 运行副本。执行 `hermes update`、重装 venv 或升级 Python 包后，`site-packages` 补丁可能被覆盖，需要重新检查上述两条保护是否仍存在。

本次备份位于：

`~/.hermes/backups/context-fix-20260718-113449/`

会话生命周期、Feishu 命令和 Role 改造前的备份位于：

`~/.hermes/backups/session-lifecycle-20260718-123741/`
