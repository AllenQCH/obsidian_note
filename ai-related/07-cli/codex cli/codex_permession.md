---
title: "Codex 权限体系：沙箱、审批、网络"
source: "ai-related/07-cli/codex cli/codex_permession.md"
author: "Allen"
published: 
created: 2026-05-07
description: "Codex 不是“天然拥有你电脑的所有权限”。它能做什么，主要由三层决定："
tags: ["obsidian-note", "tech-note", "codex", "agent", "cli", "browser-automation"]
type: "tech-note"
status: "processed"
---
# Codex 权限体系：沙箱、审批、网络
...
Codex 本质上不是一个单纯的聊天窗口，而是一个会代表你调用本机工具的 Agent。它可能要读文件、改文件、执行命令、访问网络、打开浏览器、复用 SSH key 或浏览器登录态。每一种动作背后的安全边界都不一样，所以权限也不能只用一个“允许/不允许”总开关解决。

可以把一次任务拆成几类最基本的能力：

| 能力 | 为什么需要权限 | 常见失败表现 |
| --- | --- | --- |
| 读文件 | Codex 需要读取代码、笔记、配置、日志，才能理解上下文 | 看不到目标文件、无法分析项目 |
| 写文件 | Codex 需要修改代码、生成文档、更新 Obsidian 笔记 | 提示权限不足、改动没有落盘 |
| 执行命令 | Codex 需要跑 `git`、测试、构建、脚本、CLI 工具 | 无法验证结果、无法调用本地工具 |
| 访问网络 | `git fetch/push`、下载依赖、访问 GitHub/GitLab/内网都依赖网络 | clone/push 失败、接口访问失败 |
| 使用本机凭据 | SSH key、Git credential、VPN、浏览器 cookie 都在你的本机环境里 | 终端能访问，但 Codex 访问不了 |
| 浏览器自动化 | SSO、钉钉确认、网页登录态通常依赖浏览器 profile | Playwright 打不开登录态、需要重复登录 |
| 审批越权动作 | 某些动作风险高，需要你确认后再执行 | Codex 停下来请求批准 |

这些权限分层存在，是为了同时解决两个问题：

1. **让任务能完成**：如果没有文件、网络、命令、凭据权限，Agent 只能分析，不能真正完成 clone、push、写笔记、跑测试、登录 SSO 这类操作。
2. **限制误操作半径**：如果默认给全盘读写和无限制命令执行，任何误判、恶意脚本、供应链依赖都有机会影响整台机器。

所以更准确的理解是：权限不是为了“让 Codex 更聪明”，而是为了定义 Codex 能把智能落到哪些真实工具和真实资源上。日常场景优先给最小必要权限；只有当任务确实跨目录、依赖内网、依赖浏览器登录态或需要完整本机环境时，才临时提高权限。
