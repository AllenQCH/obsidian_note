---
title: "新工作区初始化规则"
source: "Codex工作台/Codex配置和写作规范/工作区流程规则/新工作区初始化规则.md"
author: "Allen"
published: 
created: 2026-05-07
description: "进入目录时："
...
```text
/Users/heytea/Documents/myHeytea/codex-workspace
```
...
- 蓝鲸需求的可部署业务服务先同步最新 `origin/master`，再创建 `feature/<蓝鲸需求号>-<蓝鲸需求中文标题>`；中文标题使用平台原文，不缩写、不改写、不翻译。
- 已有非规范业务分支迁移时，从最新 `master` 创建规范分支，再 merge 旧需求分支并验证祖先关系。
- Maven / Gradle 公共依赖包按项目规则或用户确认使用版本分支、维护分支等目标分支，不套用蓝鲸业务服务命名。
- 允许在用户明确确认后提交和推送特性分支。
...
| 需求信号 | 项目组目录 | 说明 |
| --- | --- | --- |
| 国内 lowcost、低耗、低值易耗、费用化 | `/Users/heytea/Documents/myHeytea/code/codex-downloads/service/scm/ims` | IMS 主任务接管 |
| 国内履约、订货通履约、POF、履约单 | `/Users/heytea/Documents/myHeytea/code/codex-downloads/service/scm/dinghuotong` | 订货通主任务接管 |

路由表：

```text
/Users/heytea/Documents/myHeytea/code/codex-downloads/module-index/routing.md
```
...
```text
/Users/heytea/Documents/myHeytea/codex-workspace/.codex-launchers/launch-primary-codex.sh
```
...
```bash
prompt_file="/Users/heytea/Documents/myHeytea/codex-workspace/.codex-launchers/p23_1234-cn-pof-es-mq-log.prompt"
...
printf '%s' "/Users/heytea/Documents/myHeytea/codex-workspace/.codex-launchers/launch-primary-codex.sh /Users/heytea/Documents/myHeytea/code/codex-downloads/service/scm/dinghuotong $prompt_file" | pbcopy
