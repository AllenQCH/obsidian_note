...
- `github-web-operations` 已新增，用于复用当前 Chrome 的 GitHub 登录态执行建库、设置、上传、提交与验证；`gh` 未登录时不自动触发 CLI 授权。
...
- 2026-07-18：新增用户级 skill `/Users/heytea/.codex/skills/github-web-operations`。用途：将 GitHub 网页操作收敛为稳定 `github-web` session，默认复用当前 Chrome 登录态，区分浏览器 session 与 `gh` CLI 凭据，固化文件上传、去重、提交、公开访问/哈希验证与标签页恢复流程；同步更新 `opencli-browser-reuse` 和 `tool_github_web_operator`；启用状态：已启用；最近更新时间：2026-07-18。
- 2026-07-13：更新用户级 skill `/Users/heytea/.codex/skills/opencli-browser-reuse`。新增稳定 session `trace-log`，对应统一日志平台 `cloud-trace-log.dev.heytea.com`；同时新增本地 OpenCLI Adapter `heytea-log/topics` 和 `heytea-log/query`，将国内/海外日志查询从临时网页 `eval` 收敛为可发现、可测试的确定性命令。已通过国内/海外、生产/测试默认 Topic、自检脚本和真实只读 ERROR 字段映射验证；最近更新时间：2026-07-13。
...
- 2026-06-04：新增项目级 skill `/Users/heytea/Documents/myHeytea/code/dhtInvoice/.codex/skills/dbauto-sql-query`。用途：通过 opencli Browser Bridge 打开国内 dbauto SQL 查询页并执行只读查询，默认常用实例仅保留 3 个 MySQL：`hsp-ids`、`hsp-pof`、`hsp-pof-scm`；支持 `--list-dbs` 按实例展示数据库，`--list-tables` 按实例和库展示表，形成“选实例 -> 选库 -> 查表/SQL”的默认交互流程；已通过实例列表、库列表、表列表和生产 `center_hsp_invoice.hsp_goods_rate` 只读查询验证；最近更新时间：2026-06-04。
- 2026-06-04：删除用户级副本 `/Users/heytea/.codex/skills/dbauto-sql-query`，避免与项目级版本产生漂移；当前仅保留项目级版本；最近更新时间：2026-06-04。
- 2026-06-04：按用户确认恢复用户级 skill `/Users/heytea/.codex/skills/dbauto-sql-query`，使国内 dbauto 查询能力可在任意工作目录触发；用户级版本作为主版本，项目级 `/Users/heytea/Documents/myHeytea/code/dhtInvoice/.codex/skills/dbauto-sql-query` 可作为当前仓库副本保留；最近更新时间：2026-06-04。
...
- 2026-05-12：新增用户级 skill `/Users/heytea/.codex/skills/sso-login`。用途：让 Codex 直接发现并调用 HeyTea 统一 SSO 登录能力；来源目录为 `/Users/heytea/Documents/myHeytea/code/tool_file/agent/bug-killer/skills/sso-login` 的 wrapper，不复制 `.chromium-profile`、cookie 或 token 文件；启用状态：已启用，已通过 `quick_validate.py` 校验并可被 `superpowers-codex use-skill sso-login` 发现。
