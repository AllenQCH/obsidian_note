---
title: xuetao与my-openspec历史对比
aliases: ["xuetao与my-openspec历史对比"]
source: "conversation: Codex chat 2026-07-22"
author: 笨笨
published:
created: 2026-07-22
description: 归档 2026-07-22 时 Xuetao OpenSpec 与 my-openspec 的历史比较，不作为当前操作手册。
...
> [!warning]
> 本文是历史比较快照。当前操作统一从 [[my-openspec总览]] 进入，不要使用本文中的旧命令示例判断当前实现。

两套 OpenSpec 最大的区别不是“新旧版本功能多少”，而是**职责被重新切分了**：

- [[xuetao openspec总览]]：完整控制面，负责路由、agent、工具、工作区和执行过程。
- [[my-openspec总览]]：轻量事实面，负责项目、需求、服务绑定、结果收集和不可变归档。

所以，后来的 `AllenQCH/my-openspec` 不应该被理解成“把原始版所有能力重写了一遍”，而应该被理解成：
